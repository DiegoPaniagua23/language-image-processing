...
#SBATCH --job-name=train-char-level
...
for arch in "${ARCHES[@]}"; do
    ...
    SAVE_DIR="models/${arch_trim}_char"
    LOG_CSV="$SAVE_DIR/trainlog.csv"
    SUMMARY_PATH="$SAVE_DIR/summary.json"

    mkdir -p "$SAVE_DIR"

    echo "Iniciando entrenamiento char-level $arch_trim..."
    ...
    echo "Entrenamiento char-level $arch_trim completado."
done

echo "Todos los entrenamientos solicitados finalizaron."
